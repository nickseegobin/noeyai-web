...
function Content() {
  ...
  async function handleStart() {
    ...
    try {
      ...
      setBalance((prev) => (prev !== null ? prev - config.gemCost : prev));
      router.push(
      ...
      if (code === "noey_insufficient_tokens") {
        setError("Not enough gems. Please add more to continue.");
      } else if (code === "noey_no_exam_available") {
        setError("No exam available for this selection. Try a different subject or difficulty.");
      } else {
        setError(message);
      }
    } finally {
        ...
        {resumePrompt && activeSession && (
          <Card className="bg-noey-surface shadow-none border-2 border-noey-gem mb-5">
            <CardBody className="p-5">
              ...
              <p className="text-noey-text-muted text-sm mb-4">
                You have an unfinished {activeSession.subject} exam. Resume or start fresh?
              </p>
              <div className="flex gap-3">
                <Button onPress={handleResume} className="flex-1 bg-noey-primary text-white font-bold rounded-2xl">
                  Resume
                </Button>
                <Button onPress={handleDiscard} variant="flat" className="flex-1 bg-noey-surface-dark text-noey-text font-bold rounded-2xl">
                  Discard
                </Button>
              </div>
            </CardBody>
        ...
        {!resumePrompt && (
          <Card className="bg-noey-surface shadow-none flex-1">
            <CardBody className="p-5 flex flex-col">
              <p className="font-bold text-base text-noey-text mb-5">
                {formatDifficulty(difficulty)} Difficulty | {config.questions} Questions
              </p>
              ...
              {balance === null && (
                <div className="flex items-center justify-center gap-2 mt-4">
                  ...
                  <span className="text-noey-text-muted text-sm">Loading balance...</span>
                </div>
              )}
              ...
              <Button
                onPress={handleStart}
                isLoading={starting}
                isDisabled={!canAfford || starting || balance === null}
                className="w-full bg-noey-primary text-white font-bold h-14 rounded-2xl mt-6"
              >
                {!starting && (
                  <span className="flex items-center gap-2">
                    Start <GemIcon size={18} /> {config.gemCost}
                  </span>
                )}
              </Button>

              {!canAfford && balance !== null && (
                <p className="text-noey-gem text-sm font-medium text-center mt-2">
