...
import LeaderboardResultCard from "@/components/ui/LeaderboardResultCard";
...
// Extended result type that includes leaderboard_update from submit response
  ...
  return (
    <div className="flex flex-col min-h-dvh">
      ...
      <div className="flex-1 flex flex-col px-5 pb-8">
        <div className="mb-6">
          <span className="font-black text-xl text-noey-text">{subject} Results </span>
          ...
          <div className="flex flex-col gap-1.5 flex-1">
            {[
              ...
              <div key={l} className="flex items-center justify-between">
                <span className="text-noey-text-muted text-xs font-medium">{l}</span>
                <span className="font-bold text-sm text-noey-text">{v}</span>
              </div>
            ))}
          </div>
        </div>

        {/* Leaderboard result card — shown when submit returns leaderboard_update */}
        {result.leaderboard_update && (
          <LeaderboardResultCard
            update={result.leaderboard_update}
            standard={standard}
            term={term}
          />
        )}
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: 112, height: 112 }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-black text-xl text-noey-text">{percentage.toFixed(0)}%</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="relative h-8 bg-noey-surface rounded-2xl overflow-hidden">
        <div className="absolute inset-y-0 left-0 rounded-2xl transition-all duration-500"
          style={{ width: `${topic.percentage}%`, backgroundColor: color, opacity: 0.85 }} />
        <div className="absolute inset-0 flex items-center justify-end px-3">
          <span className="text-xs font-bold text-noey-text">
            {topic.correct}/{topic.total} &nbsp; {getScoreLabel(topic.percentage)}
          </span>
        </div>
      </div>
    </div>
