  ...
  return (
    <>
      <div className="page-container items-center">
          ...
          <div className="grid grid-cols-2 gap-6 w-full max-w-sm">
            ...
            {children.map((child) => (
              <button key={child.child_id} onClick={() => handleChildTap(child)}
                ...
                <span className="font-bold text-base text-noey-text">{child.display_name}</span>
              </button>
            ))}
            ...
            {canAddMore && children.length < 3 && (
              <button onClick={() => { window.location.href = "/parent/children/add"; }}
                ...
                <div className="text-center">
                  <p className="font-bold text-base text-noey-text">Child</p>
        ...
        <button onClick={logout} className="mt-10 text-noey-text-muted text-sm font-semibold">
          Not you? Log out
        </button>
      </div>
      ...
      <Modal isOpen={pinOpen} onOpenChange={(open) => { if (!open) { setPinOpen(false); setPin(""); setPinError(""); } }}
        ...
        <ModalContent>
          <ModalHeader className="flex flex-col items-center gap-1 pb-0">
            ...
            <p className="text-noey-text-muted text-sm font-normal">Enter your 4-digit PIN to continue</p>
          </ModalHeader>
          <ModalBody className="pb-6 pt-4 flex flex-col gap-4">
            {pinLocked ? (
              <div className="text-center py-2">
                <div className="text-4xl font-black text-noey-gem mb-2">
                  {Math.floor(lockSecs / 60)}:{String(lockSecs % 60).padStart(2, "0")}
                </div>
                <p className="text-noey-text-muted text-sm">PIN locked. Please wait.</p>
              </div>
            ) : (
              <>
